...
class Cluster:
  ...
  pvr:       {:.1f}
  """.format(
            self.medoid, len(self.members), self.seed, radius, self.successes, self.attempts, self.pvr
        )
...
""".format(
    ...
    def __init__(
        ...
        if lengths is not None:
            print("using lengths for clustering")
            indices = _np.flip(_np.argsort(lengths))
            ...
            print("using seeds for clustering")
            indices = _np.array(range(len(matrix)))
        ...
        if self.seeds is not None:
            self.seed_index = 0
            self.seed = _np.flatnonzero(self.seeds == self.seeds.min())[self.seed_index]
    ...
    def _findcluster(self):
        ...
        while threshold is None:
            ...
            if self.seeds is None:
                ...
                # breakpoint()
                min_idx = _np.flatnonzero(self.seeds == self.seeds.min())
                self.seed_index = self.seed_index % len(min_idx)
                # print("trying seed", self.seed_index, "of", len(min_idx), self.seeds.min(), self.seeds[self.seed_index], len(self.matrix), len(self.seeds), "clusters:", self.nclusters)
                self.seed = min_idx[self.seed_index % len(min_idx)]
                self.seed_index += 1
...
def cluster(
    ...
    it = ClusterGenerator(matrix, maxsteps, windowsize, minsuccesses, destroy, normalized, cuda, seeds, contig_lens)
    for cluster in it:
...
def pairs(clustergenerator, labels):
    ...
    if len(labels) < maxindex:
        raise ValueError(
            "Cluster generator contains point no {}, " "but was given only {} labels".format(maxindex, len(labels))
        )
