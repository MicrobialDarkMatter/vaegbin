...
from genelib.layers import BiasLayer, LAF, GraphAttention

class TH:
    def __init__(self, model, lr=0.01, lambda_vae=0.1, all_different_idx=None, all_same_idx=None,):
    ...
    def train_unsupervised(self, idx):
        with tf.GradientTape() as tape:
            ...
            pos_loss = tf.reduce_mean(tf.keras.losses.binary_crossentropy(tf.ones_like(positive_pairwise), positive_pairwise, from_logits=True))
            neg_loss = tf.reduce_mean(tf.keras.losses.binary_crossentropy(tf.zeros_like(negative_pairs), negative_pairs, from_logits=True))
    ...
    def train_unsupervised_vae(self, idx):
        with tf.GradientTape() as tape:
            ...
            z = eps * std + mu
            kl = 0.5 / n_nodes * tf.reduce_mean(tf.reduce_sum(1. + 2. * log_std - mu**2 - std**2, axis=1))
            
            pairwise_similarity = tf.matmul(z, z, transpose_b=True)
            y = tf.reshape(self.dense_adj, [-1])
            y_hat = tf.reshape(pairwise_similarity, [-1])
            
            loss = self.norm * tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=y_hat, labels=y, pos_weight=self.pos_weight))
    ...
    def sample_idx(idx,n):
        ...
        random_idx = tf.random.uniform(shape=(n,), minval=0, \
                                       maxval=N, dtype=tf.int64)
...
class GAT(Model):
    def __init__(self, features, labels, adj, n_labels=None, \
        ...
        # second sage layer
        if conv_last:
            x = GraphAttention(n_labels,dropout_rate=0.1,attn_heads=1,activation='linear', attn_heads_reduction='average')([adj_in, x])
        else:
            x = Dense(n_labels, use_bias=True)(x)
...
class GATLAF(Model):
    def __init__(self, features, labels, adj, n_labels=None, \
        ...
        x = node_in
        for l in range(layers):
            x = GraphAttention(hidden_units, dropout_rate=0.1,attn_heads=1,activation='linear', laf_units=4)([adj_in, x])
            x = BatchNormalization(momentum=0.9, epsilon=1e-5)(x)
            x = Activation('relu')(x)
            x = Dropout(0.5)(x)
        ...
        # second sage layer
        if conv_last:
            x = GraphAttention(n_labels,dropout_rate=0.1,attn_heads=1,activation='linear', attn_heads_reduction='average', laf_units=4)([adj_in, x])
        else:
            x = Dense(n_labels, use_bias=True)(x)
